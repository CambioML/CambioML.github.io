export const ko = {
  ...
  homepage: {
    hero: {
      ...
      description: 'OCR 사용자의 80%가 글루 코드를 유지하는 데 지쳤습니다. 탁월한 정확도, 완전한 프라이버시, 구성 가능한 옵션으로 시간을 절약하세요:',
      features: [
        '개인 정보 식별 정보 제거,',
        '표와 차트 추출,',
        '각주와 헤더 유지,'
      ...
      steps: [
        {
          ...
          description: '문서를 파싱하려면 \'파일 업로드\'를 클릭하여 문서를 드래그 앤 드롭하거나 클립보드에서 스크린샷을 붙여넣으세요. 시작을 돕기 위한 샘플 문서도 제공됩니다.'
        },
        {
          title: '02',
          subtitle: '파싱 및 프라이버시 설정 편집',
          description: 'AnyParser는 PII(개인 식별 정보), 각주, 표 등 다양한 정보를 자동으로 분류합니다. 필요한 데이터를 간단히 내보내세요!'
        },
        {
          title: '03',
          subtitle: '결과를 시스템에 내보내기',
          description: 'HTML, Excel, JSON 또는 비즈니스 워크플로우에 맞춘 데이터베이스 스키마 등 선호하는 형식으로 데이터를 다운로드하세요.'
        }
      ]
    },
    banners: [
      {
        title: '정확한 데이터 파싱',
        description: 'AnyParser 플레이그라운드는 직관적이고 빠르며 간단합니다. 지금 인터페이스를 사용해보고 하루를 여유롭게 보내세요.',
        actionLabel: '무료로 사용해보기'
      },
      {
        title: 'AnyParser로 구축하기',
        description: 'AnyParser 플레이그라운드는 직관적이고 빠르며 정말 간단합니다. 지금 인터페이스를 사용해보고 하루를 여유롭게 보내세요.',
  ...
  playground: {
    ...
    disclaimers: {
      ...
      fileSize: '**최대 파일 크기는 10MB입니다. 이 페이지를 새로 고치면 모든 파일과 처리된 데이터가 삭제됩니다. 우리는 데이터를 저장하지 않습니다.',
    ...
    info: {
      plainText: {
        ...
        howTo: '파일을 업로드하고 선택한 후 \'Plain Text\' 흐름을 실행하면 파일의 콘텐츠를 추출하여 Markdown 형식으로 반환합니다.',
        nextSteps: '다음 단계',
        nextStepsDescription: '추출된 테이블에서 원시 Markdown을 텍스트 파일로 다운로드하거나 JSON을 다운로드할 수 있습니다.'
      },
      table: {
        title: '테이블만 추출',
        description: 'AnyParser를 사용하면 파일에서 테이블을 추출하고 특정 키를 추출하여 데이터베이스 스키마에 매핑할 수 있습니다.',
        step1: '1. 테이블 추출',
        step1Description: '먼저 파일에서 테이블을 추출해야 합니다. AnyParser는 모든 테이블을 HTML 형식으로 추출합니다.\n\n추출 후 원시 HTML을 다운로드하거나 모든 테이블을 Excel 파일로 다운로드할 수 있습니다.',
        step2: '2. 매핑할 테이블 선택',
        step2Description: '테이블 추출을 실행한 후 데이터를 추출하려는 테이블을 선택하세요.\n\n이 섹션에서는 각 테이블의 HTML을 미리 볼 수 있는 기능도 제공합니다.',
        step3: '3. 스키마 매핑',
        step3Description: '테이블을 추출하고 선택한 후 추출하려는 키를 추가할 수 있습니다. 매핑된 키를 제거하려면 상자에서 (X)를 클릭하세요. 입력 키를 제거하려면 상자에서 (X) 아이콘을 클릭할 수 있습니다.\n\n키를 추가한 후 스키마 매핑 기능을 실행할 수 있습니다. 매핑 후에는 편집 연필 아이콘을 클릭하여 매핑된 키를 편집할 수 있습니다.\n\n\'스키마 매핑\'을 다시 클릭하면 매핑된 키 없이 입력 키만 다시 실행됩니다. 새 매핑된 키를 생성하려면 먼저 매핑된 키를 제거한 후 X를 클릭하세요.',
        nextSteps: '다음 단계',
        nextStepsDescription: '출력에 만족하면 CSV 또는 JSON을 다운로드할 수 있습니다. 이는 최소한 하나의 입력 키에 비어 있지 않은 매핑된 키가 있는 경우에만 활성화됩니다.'
      },
      keyValuePair: {
        title: '키-값 쌍 추출',
        description: 'AnyParser를 사용하면 송장, 계약서 또는 양식과 같은 원시 비구조화 데이터에서 구조화된 키-값 쌍을 식별하고 추출할 수 있습니다.',
        howTo: '파일을 업로드하고 선택한 후 \'Key-Value Pair\' 흐름을 실행하면 파일을 분석하고 주요 필드를 식별하며 해당 값을 구조화된 JSON 형식으로 추출합니다.',
        nextSteps: '다음 단계',
        nextStepsDescription: '추출된 키-값 쌍을 JSON 파일로 다운로드하거나 API를 통해 워크플로우와 직접 통합하여 추가 처리를 자동화할 수 있습니다.'
      }
    },
    comingSoon: '곧 출시 예정',
  ...
  solutionPages: {
    compliance: {
      ...
      section: {
        ...
        paragraph: '개인 LLM의 준수 또는 위험이 걱정되십니까? 우리는 규제 준수 및 위험 관리를 위한 전문 준수 LLM을 제공합니다. 이 시스템은 규제 변경 사항을 자동으로 모니터링하고 내부 정책 및 통제와 일치시켜 중요한 규정 및 요구 사항에 대한 적시 추적, 대응 및 보고를 보장합니다.'
      },
      useCases: [
        {
          title: '준수 사용 사례 1',
      ...
      autorater: {
        ...
        section: {
          ...
          paragraph: '개인 LLM의 AutoRater 또는 위험이 걱정되십니까? 우리는 규제 AutoRater 및 위험 관리를 위한 전문 AutoRater LLM을 제공합니다. 이 시스템은 규제 변경 사항을 자동으로 모니터링하고 내부 정책 및 통제와 일치시켜 중요한 규정 및 요구 사항에 대한 적시 추적, 대응 및 보고를 보장합니다.'
        }
      }
    },
    finance: {
      title: '통찰력을 발견하기 위한 맞춤형 AI 비서',
      description: '잘못 가격이 책정된 주식을 더 빠르게 찾으세요.\n실제 데이터를 사용하여 투자 결정을 설명하세요.\n자신의 독점 AI 에이전트를 완전히 소유하고 제어하세요.',
      iconSection: {
    ...
    researchDevelopment: {
      ...
      description: '데이터 정리에 소요되는 시간을 최대 90% 줄이세요\n최신 연구를 최신 상태로 유지하세요\nR&D에서 알려지지 않은 것을 쉽게 발견하세요',
      features: [
        {
          title: '다중 소스 데이터에서 알 수 없는 통찰력에 접근',
          text: '• 데이터 정리에 소요되는 시간을 최대 90% 줄이세요\n• R&D에서 알려지지 않은 것을 쉽게 발견하세요\n• 자신의 독점 AI 에이전트를 완전히 소유하고 제어하세요'
